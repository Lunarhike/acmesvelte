<script lang="ts">
	...
	import InvoiceStatus from './components/InvoiceStatus.svelte';
	import UpdateInvoice from './components/UpdateInvoice.svelte';
	import DeleteInvoice from './components/DeleteInvoice.svelte';

	export let filteredInvoiceId: string;
	...
	const query = createQuery({
		queryKey: ['filteredInvoices', filteredInvoiceId],
		queryFn: getFilteredInvoices
	});
</script>
...
{#if $query.status === 'success'}
	<div class="w-full">
		...
		<div class="mt-4 flex items-center justify-between gap-2 md:mt-8"></div>
		<div class="mt-6 flow-root">
			<div class="inline-block min-w-full align-middle">
				<div class="rounded-lg bg-gray-50 p-2 md:pt-0">
					<div class="md:hidden">
						{#each $query.data.filteredInvoices as invoice (invoice.id)}
							<div class="mb-2 w-full rounded-md bg-white p-4">
								...
								<div class="flex w-full items-center justify-between pt-4">
									<div>
										<p class="text-xl font-medium">{formatCurrency(invoice.amount)}</p>
						...
						<tbody class="bg-white">
							{#each $query.data.filteredInvoices as invoice (invoice.id)}
								...
								>
									...
									<td class="whitespace-nowrap px-3 py-3">{formatCurrency(invoice.amount)}</td>
									<td class="whitespace-nowrap px-3 py-3">{formatDateToLocal(invoice.date)}</td>
									<td class="whitespace-nowrap px-3 py-3"
										><InvoiceStatus status={invoice.status} /></td
									>
									<td class="whitespace-nowrap py-3 pl-6 pr-3">
